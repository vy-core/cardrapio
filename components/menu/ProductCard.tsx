...
import type { Produto, Adicionais } from "@/types";
...
export function ProductCard({ produto }: ProductCardProps) {
	...
	const [selectedToppings, setSelectedToppings] = useState<Adicionais[]>([]);
	...
	const handleToppingToggle = (topping: Adicionais) => {
	...
	const toppingsPrice = selectedToppings.reduce((sum, t) => sum + t.preco, 0);
	const totalItemPrice = (produto.preco + toppingsPrice) * quantity;
		...
		<div className="flex flex-col max-h-full overflow-hidden w-full">
			<div className="px-5 py-6 space-y-6 overflow-y-auto flex-1">
				...
				<div className="space-y-3">
					...
					{produto.grupos_adicionais.map(grupoAd => (
						<div key={grupoAd.id} className="space-y-2">
							<Label className="text-[14px] font-bold text-gray-700">
								{grupoAd.grupo.nome}
							</Label>
							<div className="flex flex-col gap-2">
								{grupoAd.grupo.adicionais.map((adicional) => {
									const isChecked = !!selectedToppings.find(t => t.id === adicional.id);
									return (
										<Label
											key={adicional.id}
										...
										>
											<div className="flex items-center gap-3">
												<Checkbox
													checked={isChecked}
													onCheckedChange={() => handleToppingToggle(adicional)}
												/>
												<span className="text-[14px] font-medium text-gray-700">{adicional.nome}</span>
											</div>
											<span className="text-[13px] font-bold text-brand-600">+ {formatPrice(adicional.preco)}</span>
										</Label>
									);
								})}
							</div>
		...
		<>
			...
			>
				...
				<div className="w-full h-32 rounded-2xl bg-linear-to-br from-brand-50 to-cream-100 flex items-center justify-center text-5xl select-none shadow-sm cursor-pointer" onClick={handleOpen}>
					{produto.image_url ? (
						<img src={produto.image_url} alt={produto.nome} className="w-full h-full object-cover rounded-2xl" />
					) : (
						produto.categoria_id === "cat-1" ? "🟣"
							: produto.categoria_id === "cat-2" ? "🍦"
								: produto.categoria_id === "cat-3" ? "🍧"
									: produto.categoria_id === "cat-4" ? "🥤"
										: "🍫"
					)}
				</div>
				...
				<Drawer open={isOpen} onOpenChange={setIsOpen}>
					<DrawerContent className="bg-white p-0 border-none outline-none overflow-hidden max-h-[90vh]">
						<DrawerHeader className="px-5 pt-6 pb-4 border-b border-gray-100 text-left bg-white z-10 relative">
							<DrawerTitle className="text-xl font-bold text-gray-900 pr-8 leading-tight">{produto.nome}</DrawerTitle>
							<p className="text-[14px] text-gray-500 mt-2 line-clamp-3">{produto.descricao}</p>
						</DrawerHeader>
						{FormContent}
					</DrawerContent>
				</Drawer>
				...
				<Dialog open={isOpen} onOpenChange={setIsOpen}>
					<DialogContent className="p-0 max-w-lg rounded-2xl gap-0 h-4/5">
						<DialogHeader className="px-5 pt-6 pb-4 border-b border-gray-100 text-left bg-white z-10 relative rounded-full">
							<DialogTitle className="text-xl font-bold text-gray-900 pr-8 leading-tight">{produto.nome}</DialogTitle>
							<DialogDescription className="text-[14px] text-gray-500 mt-2 line-clamp-3">{produto.descricao}</DialogDescription>
						</DialogHeader>
						{FormContent}
					</DialogContent>
				</Dialog>
